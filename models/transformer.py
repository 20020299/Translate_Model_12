...
class Transformer(nn.Module):
    ...
    def __init__(self, mode=None, model_dir=None, config=None):
        ...
        if ('train_data_location' in opt or 'train_data_location' in opt.get("data", {})):
            ...
            self.loader = DefaultLoader(data_opt['train_data_location'],
                                        eval_path=data_opt.get('eval_data_location', None),
                                        language_tuple=(data_opt["src_lang"], data_opt["trg_lang"]), option=opt)
        elif ('data' in opt):
        ...
        with torch.no_grad():
            ...
            for batch in valid_iter:
                ...
                src_mask, trg_mask = create_masks(src, trg_input, src_pad, trg_pad,
                                                  opt.get('device', const.DEFAULT_DEVICE))
                preds = self(src, trg_input, src_mask, trg_mask)
    ...
    def translate_batch(self, batch_sentences, src_lang=None, trg_lang=None, output_tokens=False,
        ...
        if (input_max_length is None):
            input_max_length = self.config.get("input_max_length", const.DEFAULT_INPUT_MAX_LENGTH)
        translated_batch = self.decode_strategy.translate_batch(batch_sentences, trg_lang=trg_lang,
                                                                src_size_limit=input_max_length, output_tokens=True,
                                                                debug=False)
        ...
        for epoch in range(checkpoint_idx, opt['epochs']):
            ...
            for i, batch in enumerate(self.train_iter):
                ...
                if (i + 1) % opt['printevery'] == 0:
                    ...
                    logging.info(
                        'epoch: {:03d} - iter: {:05d} - train loss: {:.4f} - time elapsed/per batch: {:.4f} {:.4f}'.format(
                            epoch, i + 1, avg_loss, et, et / opt['printevery']))
                    total_loss = 0
                    s = time.time()
            ...
            if (epoch + 1) % opt['save_checkpoint_epochs'] == 0 and model_dir is not None:
                ...
                saver.save_and_clear_model(model, model_dir, checkpoint_idx=epoch + 1,
                ...
                best_model_score = saver.save_model_best_to_path(model, model_dir, best_model_score, bleuscore,
                ...
                logging.info(
                    'epoch: {:03d} - iter: {:05d} - valid loss: {:.4f} - bleu score: {:.4f} - full evaluation time: {:.4f}'.format(
                        epoch, i, valid_loss, bleuscore, time.time() - s))
            else:
                # print('epoch: {:03d} - iter: {:05d} - valid loss: {:.4f} - validation time: {:.4f}'.format(epoch, i, valid_loss, time.time() - s))
                logging.info(
                    'epoch: {:03d} - iter: {:05d} - valid loss: {:.4f} - validation time: {:.4f}'.format(epoch, i,
                                                                                                         valid_loss,
                                                                                                         time.time() - s))
    ...
    def prepare_serve(self, serve_path, model_dir=None, check_trace=True, **kwargs):
        ...
        sample_src, sample_trg = torch.randint(fake_range, (fake_batch, fake_srclen), dtype=torch.long), torch.randint(
            fake_range, (fake_batch, fake_trglen), dtype=torch.long)
        sample_src_mask, sample_trg_mask = torch.rand(fake_batch, 1, fake_srclen) > 0.5, torch.rand(fake_batch,
                                                                                                    fake_trglen,
                                                                                                    fake_trglen) > 0.5
        sample_src, sample_trg, sample_src_mask, sample_trg_mask = [t.to(self.device) for t in
                                                                    [sample_src, sample_trg, sample_src_mask,
                                                                     sample_trg_mask]]
        ...
        needed_fn = {'forward': (sample_src, sample_trg, sample_src_mask, sample_trg_mask),
                     "encode": (sample_src, sample_src_mask),
                     "decode": (sample_trg, sample_encoded, sample_src_mask, sample_trg_mask),
                     "to_logits": sample_before_logits}
